...
Run `npx hyperframes@0.7.5 tts --list` for all 54 voices (8 languages).
...
```bash
npx hyperframes@0.7.5 tts "La reunión empieza a las nueve" --voice ef_dora --output es.wav
npx hyperframes@0.7.5 tts "今日はいい天気ですね" --voice jf_alpha --output ja.wav
```

Use `--lang` only to override auto-detection (e.g. stylized accents):

```bash
npx hyperframes@0.7.5 tts "Hello there" --voice af_heart --lang fr-fr --output accented.wav
```
...
```bash
npx hyperframes@0.7.5 tts "Your script here" --voice af_nova --output narration.wav
npx hyperframes@0.7.5 tts script.txt --voice bf_emma --output narration.wav
```
...
```bash
npx hyperframes@0.7.5 tts script.txt --voice af_heart --output narration.wav
npx hyperframes@0.7.5 transcribe narration.wav  # → transcript.json with word-level timestamps
```
